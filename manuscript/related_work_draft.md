...
**Status:** Research-track draft, 2026-08-27. This document is not submission-ready. Claims marked with brackets require further verification or measurement before inclusion in the final manuscript.
...
Phoenix is an open-source, self-hostable AI observability and evaluation platform built on OpenTelemetry and OpenInference, with auto-instrumentation, real-time troubleshooting, trace visualization, evaluations, datasets, and experiments. AgentScope's existing baseline measured instrumentation overhead against one local Phoenix configuration, but it did not compare time-to-visible-structure or time-to-anomaly because Phoenix has no configured equivalent of AgentScope's six-rule engine in that experiment. Phoenix is a strong baseline, and claims of feature or timing superiority remain [TBD—requires the controlled comparison specified in the evaluation guide].
...
Phoenix likewise provides open-source/self-hosted tracing, real-time troubleshooting, auto-instrumentation through OpenInference, trace visualization, and evaluation workflows [13]. AgentScope has one measured local overhead comparison against Phoenix: on the recorded paired workloads, AgentScope's instrumentation overhead was lower in that configuration (CHANGELOG [2026-08-22 01:20]). This result is configuration-specific and does not establish broader performance or diagnostic superiority. The current baseline did not configure an equivalent Phoenix detector, did not measure first-visible graph time, and did not measure time-to-diagnosis; those comparisons remain [TBD—pending the controlled comparative study]. Langfuse was not benchmarked at all.
...
The claim is falsifiable through four comparisons: graph-parent/owner fidelity across nested and concurrent custom-agent executions; detection accuracy and delay on labeled failure sequences; wire-level absence of protected content and key material under redaction; and controlled time-to-visible-evidence/time-to-diagnosis comparisons with Langfuse and Phoenix. Until those experiments are complete, the manuscript should call the paragraph above the **proposed contribution**, not a proven superiority result.
